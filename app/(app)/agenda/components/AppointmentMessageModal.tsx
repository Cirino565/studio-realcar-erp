...
const DEFAULT_TEMPLATE: WhatsAppTemplateType = "reminder";
...
function AppointmentMessageContent({
  appointment,
  onClose,
}: ContentProps) {
  const [selectedTemplate, setSelectedTemplate] =
    useState<WhatsAppTemplateType>(DEFAULT_TEMPLATE);
  const [message, setMessage] = useState(() =>
    gerarMensagem(appointment, DEFAULT_TEMPLATE),
  );
  ...
  const whatsappUrl = useMemo(
    ...
    [appointment.cliente.telefone, appointment.cliente.whatsapp, message],
  );
  ...
  function selecionarModelo(template: WhatsAppTemplateType) {
    setSelectedTemplate(template);
    setMessage(gerarMensagem(appointment, template));
    setCopied(false);
  }
      ...
      <header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-white/[0.035] sm:gap-6 sm:p-6">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm leading-5 text-slate-500 dark:text-slate-400">
            Escolha o objetivo, revise o texto e personalize antes de abrir o
            WhatsApp.
          </p>
        </div>
        ...
        <main className="p-4 sm:p-6">
          ...
          {!hasPhone ? (
            <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100">
              Cadastre um telefone ou WhatsApp para abrir esta conversa. Ainda
              é possível copiar o texto.
            </div>
          ) : null}
...
export default function AppointmentMessageModal({
  ...
  return (
    <div className="app-modal-backdrop fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto bg-slate-950/55 px-3 py-4 backdrop-blur-sm sm:items-center sm:px-4">
